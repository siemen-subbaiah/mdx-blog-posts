...
description: "React Server Components is one of the most talked subjects in the react/next.js community, so let's see what is the hype about"
---
...
### Basic example of RSC in Next.js 13

```
const page = async ({ params }: { params: { slug: string } }) => {
  const content = await getSinglePost(params.slug);
